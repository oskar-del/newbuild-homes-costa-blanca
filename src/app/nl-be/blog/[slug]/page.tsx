...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const tableOfContents = isStructured && structured.sections
    ? [...structured.sections.map((s: ArticleSection) => ({ id: slugify(s.title), title: s.title })),
       ...structured.faqs?.length ? [{ id: 'faqs', title: 'Veelgestelde vragen' }] : []]
    : [];
      ...
      <main className="min-h-screen bg-warm-50">
        ...
        <section className="relative bg-primary-900 py-12 md:py-16">
          <div className="max-w-4xl mx-auto px-6">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-warm-400 text-sm">{article.readTime} min leestijd</span>
            </div>
            ...
            <div className="flex items-center gap-4 text-warm-400 text-sm">
              ...
              <span>{new Date(article.publishedAt).toLocaleDateString('nl-BE', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
            </div>
          </div>
        </section>
            ...
            <div className="grid lg:grid-cols-3 gap-12">
                    ...
                    <div className="prose prose-lg max-w-none">
                      {structured.quickAnswer && (
                        <div className="mb-8 bg-gradient-to-br from-primary-50 to-accent-50 rounded-sm p-5 border border-accent-200">
                          <div className="flex items-center gap-2 mb-3">
                            ...
                            <span className="text-primary-900 font-bold text-sm uppercase tracking-wide">Snel antwoord</span>
                          </div>
                      ...
                      {structured.faqs?.length > 0 && (
                        <section id="faqs" className="mt-10 scroll-mt-20">
                          <h2 className="text-2xl font-bold text-primary-900 mb-6">Veelgestelde vragen</h2>
                      ...
                      <div className="mt-8 p-5 bg-primary-900 rounded-sm text-white lg:hidden">
                        <h3 className="font-semibold text-lg mb-2">Hulp nodig?</h3>
                        <div className="flex flex-col gap-3">
                          ...
                          <Link href={`/${LANG}/contact`} className="block w-full bg-white/10 hover:bg-white/20 text-white text-center py-3 rounded font-medium transition-colors border border-white/20">Boek videogesprek</Link>
                        </div>
                      </div>
                    </div>
                  ) : (
              ...
              <aside className="lg:col-span-1 space-y-6">
                <div className="bg-primary-900 p-6 rounded-sm text-white sticky top-6">
                  ...
                  <p className="text-warm-300 text-sm mb-4">Persoonlijke aanbevelingen voor u</p>
                  <div className="space-y-3">
                    ...
                    <Link href={`/${LANG}/contact`} className="flex items-center justify-center gap-2 w-full bg-white/10 hover:bg-white/20 text-white py-3 rounded-sm font-medium transition-colors border border-white/20">Boek videogesprek</Link>
                  </div>
                </div>
                <div className="bg-white p-6 rounded-sm border border-warm-200">
                  <h3 className="font-semibold text-primary-900 mb-4">Ontdek woningen</h3>
                  <ul className="space-y-3">
                    <li><Link href={`/${LANG}/developments`} className="text-accent-600 hover:text-accent-700 font-medium">Bekijk projecten</Link></li>
                    <li><Link href={`/${LANG}/areas`} className="text-accent-600 hover:text-accent-700 font-medium">Ontdek gebieden</Link></li>
                    <li><Link href={`/${LANG}/golf`} className="text-accent-600 hover:text-accent-700 font-medium">Golfwoningen</Link></li>
                  </ul>
                </div>
                <div className="bg-gradient-to-br from-accent-500 to-accent-600 p-6 rounded-sm text-white">
                  <h3 className="font-semibold text-lg mb-2">Actuele prijzen</h3>
                  <p className="text-white/90 text-sm mb-4">Verzoek onze prijslijst</p>
                  <Link href={`/${LANG}/contact`} className="block w-full bg-white text-accent-600 hover:bg-warm-50 text-center py-3 rounded-sm font-semibold transition-colors">Prijslijst aanvragen</Link>
                </div>
              </aside>
            </div>
          </div>
        ...
        {relatedArticles.length > 0 && (
          <section className="py-12 bg-warm-100">
            <div className="max-w-7xl mx-auto px-6">
              <h2 className="text-2xl font-bold text-primary-900 mb-8 text-center">Meer artikelen</h2>
        ...
        <section className="py-12 bg-primary-900">
          <div className="max-w-4xl mx-auto px-6 text-center">
            <h2 className="text-2xl md:text-3xl font-semibold text-white mb-3">Klaar om uw droomhuis te vinden?</h2>
            <p className="text-warm-300 mb-8">Neem vandaag contact met ons op</p>
            <div className="flex flex-wrap justify-center gap-4">
              <Link href={`/${LANG}/contact`} className="bg-accent-500 hover:bg-accent-600 text-white font-semibold px-8 py-4 rounded-sm transition-colors">Gratis adviesgesprek</Link>
              <Link href={`/${LANG}/developments`} className="bg-white/10 hover:bg-white/20 text-white font-semibold px-8 py-4 rounded-sm transition-colors border border-white/20">Bekijk projecten</Link>
            </div>
          </div>
        </section>
      </main>
