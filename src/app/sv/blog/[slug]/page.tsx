...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!article) return { title: 'Artikel inte hittad' };
...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const tableOfContents = isStructured && structured.sections
    ? [
        ...structured.sections.map((section: ArticleSection) => ({
          id: slugify(section.title),
          title: section.title,
        })),
        ...structured.faqs?.length ? [{ id: 'faqs', title: 'Vanliga frågor' }] : [],
      ]
    : [];
      ...
      <main className="min-h-screen bg-warm-50">
        ...
        <section className="relative bg-primary-900 py-12 md:py-16 overflow-hidden">
          ...
          <div className="max-w-4xl mx-auto px-6 relative z-10">
            <nav className="text-warm-400 text-sm mb-6">
              <Link href={`/${LANG}`} className="hover:text-white transition-colors">Hem</Link>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-warm-400 text-sm">{article.readTime} min läsning</span>
            </div>
            ...
            <div className="flex items-center gap-4 text-warm-400 text-sm">
              ...
              <span>{new Date(article.publishedAt).toLocaleDateString('sv-SE', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
            </div>
          </div>
        </section>
            ...
            <div className="grid lg:grid-cols-3 gap-12">
                  ...
                  {isStructured ? (
                    <div className="prose prose-lg max-w-none">
                      ...
                      {structured.quickAnswer && (
                        <div className="mb-8 bg-gradient-to-br from-primary-50 to-accent-50 rounded-sm p-5 border border-accent-200">
                          <div className="flex items-center gap-2 mb-3">
                            ...
                            <span className="text-primary-900 font-bold text-sm uppercase tracking-wide">Snabbsvar</span>
                          </div>
                      ...
                      {tableOfContents.length > 3 && (
                        <nav className="mb-10 bg-warm-50 rounded-sm border border-warm-200" aria-label="Innehållsförteckning">
                          <details className="group" open>
                            ...
                            <ol className="px-5 pb-5 space-y-2 list-decimal list-inside text-warm-700">
                              {tableOfContents.map((item: any, index: number) => (
                                <li key={index}>
                                  <a href={`#${item.id}`} className="text-accent-600 hover:text-accent-700 hover:underline transition-colors">{item.title}</a>
                                </li>
                              ))}
                            </ol>
                          </details>
                      ...
                      {structured.sections?.map((section: ArticleSection, index: number) => {
                        ...
                        const borderColor = 'border-accent-500';
                        return (
                          <div key={index}>
                            <section
                              id={slugify(section.title)}
                      ...
                      {structured.faqs && structured.faqs.length > 0 && (
                        <section id="faqs" className="mt-10 scroll-mt-20">
                          <h2 className="text-2xl font-bold text-primary-900 mb-6">Vanliga frågor</h2>
                          <div className="space-y-4">
                            {structured.faqs.map((faq: FAQ, index: number) => (
                      ...
                      <div className="mt-8 p-5 bg-primary-900 rounded-sm text-white lg:hidden">
                        <h3 className="font-semibold text-lg mb-2">Behöver du hjälp?</h3>
                        <p className="text-warm-300 text-sm mb-4">Få personliga rekommendationer från vårt team</p>
                        <div className="flex flex-col gap-3">
                          ...
                          <Link href={`/${LANG}/contact`} className="block w-full bg-white/10 hover:bg-white/20 text-white text-center py-3 rounded font-medium transition-colors border border-white/20">Boka videosamtal</Link>
                        </div>
                      </div>
                    </div>
                  ) : (
              ...
              <aside className="lg:col-span-1 space-y-6">
                <div className="bg-primary-900 p-6 rounded-sm text-white sticky top-6">
                  <h3 className="font-semibold text-lg mb-3">Behöver du hjälp?</h3>
                  <p className="text-warm-300 text-sm mb-4">Få personliga rekommendationer</p>
                  <div className="space-y-3">
                    ...
                    <Link href={`/${LANG}/contact`} className="flex items-center justify-center gap-2 w-full bg-white/10 hover:bg-white/20 text-white py-3 rounded-sm font-medium transition-colors border border-white/20">Boka videosamtal</Link>
                  </div>
                </div>
                <div className="bg-white p-6 rounded-sm border border-warm-200">
                  <h3 className="font-semibold text-primary-900 mb-4">Utforska fastigheter</h3>
                  <ul className="space-y-3">
                    <li><Link href={`/${LANG}/developments`} className="text-accent-600 hover:text-accent-700 font-medium">Bläddra projekt</Link></li>
                    <li><Link href={`/${LANG}/areas`} className="text-accent-600 hover:text-accent-700 font-medium">Utforska områden</Link></li>
                    <li><Link href={`/${LANG}/golf`} className="text-accent-600 hover:text-accent-700 font-medium">Golffastigheter</Link></li>
                  </ul>
                </div>
                <div className="bg-gradient-to-br from-accent-500 to-accent-600 p-6 rounded-sm text-white">
                  <h3 className="font-semibold text-lg mb-2">Få senaste priser</h3>
                  <p className="text-white/90 text-sm mb-4">Prislista och aktuella erbjudanden</p>
                  <Link href={`/${LANG}/contact`} className="block w-full bg-white text-accent-600 hover:bg-warm-50 text-center py-3 rounded-sm font-semibold transition-colors">Begär prislista</Link>
                </div>
              </aside>
            </div>
          </div>
        ...
        {relatedArticles.length > 0 && (
          <section className="py-12 bg-warm-100">
            <div className="max-w-7xl mx-auto px-6">
              <h2 className="text-2xl font-bold text-primary-900 mb-8 text-center">Fler artiklar</h2>
                  ...
                  <Link key={related.slug} href={`/${LANG}/blog/${related.slug}`} className="bg-white rounded-sm overflow-hidden border border-warm-200 hover:shadow-lg transition-shadow group">
                    <div className="p-6">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="text-warm-400 text-xs">{related.readTime} min</span>
                      </div>
        ...
        <section className="py-12 bg-primary-900">
          <div className="max-w-4xl mx-auto px-6 text-center">
            <h2 className="text-2xl md:text-3xl font-semibold text-white mb-3">Redo att hitta ditt hem?</h2>
            <p className="text-warm-300 mb-8 max-w-2xl mx-auto">Boka en gratis konsultation med vårt team och få personliga rekommendationer</p>
            <div className="flex flex-wrap justify-center gap-4">
              <Link href={`/${LANG}/contact`} className="bg-accent-500 hover:bg-accent-600 text-white font-semibold px-8 py-4 rounded-sm transition-colors">Boka gratis konsultation</Link>
              <Link href={`/${LANG}/developments`} className="bg-white/10 hover:bg-white/20 text-white font-semibold px-8 py-4 rounded-sm transition-colors border border-white/20">Bläddra projekt</Link>
            </div>
          </div>
        </section>
      </main>
