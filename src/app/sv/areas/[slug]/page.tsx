...
export async function generateStaticParams() {
  const slugs = getTranslatedAreaSlugs('sv');
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `https://newbuildhomescostablanca.com/sv/areas/${slug}`,
      languages: {
...
export default async function LocalizedAreaPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const strings = getAreaStrings('sv');
