...
export async function generateStaticParams() {
  const slugs = getTranslatedAreaSlugs('pl');
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `https://newbuildhomescostablanca.com/pl/areas/${slug}`,
      languages: {
...
export default async function LocalizedAreaPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const strings = getAreaStrings('pl');
