...
export const metadata: Metadata = {
  title: 'Costa Blanca Poradniki: Kupować, Mieszkać & Styl Życia | Porady dla Polaków',
  description: 'Twój przewodnik po życiu nad Morzem Śródziemnym. Przewodniki regionów, przewodniki plaż, porady dotyczące kupna, życie golfowe i wszystko, co musisz wiedzieć o życiu na Costa Blance dla Polaków.',
  openGraph: {
    title: 'Costa Blanca Poradniki: Kupować, Mieszkać & Styl Życia',
    description: 'Twój przewodnik po życiu nad Morzem Śródziemnym w Hiszpanii.',
    type: 'website',
...
const SLUG_IMAGES: Record<string, string> = {
  ...
  'polskie-zycie-na-costa-blanca': 'https://images.unsplash.com/photo-1519046904884-53103b34b206?w=800&h=600&fit=crop',
  'kupowanie-domu-w-hiszpanii-jako-polak': 'https://images.unsplash.com/photo-1560518883-ce09059eeffa?w=800&h=600&fit=crop',
  'golf-w-hiszpanii-dla-polskich-golfistrów': 'https://images.unsplash.com/photo-1587174486073-ae5e5cff23aa?w=800&h=600&fit=crop',
  'warszawskie-ceny-vs-costa-blanca': 'https://images.unsplash.com/photo-1449844908441-8829872d2607?w=800&h=600&fit=crop',
};

const CATEGORY_IMAGES: Record<string, string> = {
  'Styl życia': 'https://images.unsplash.com/photo-1540541338287-41700207dee6?w=800&h=600&fit=crop',
  'Przewodnik regionu': 'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=800&h=600&fit=crop',
  'Poradnik kupna': 'https://images.unsplash.com/photo-1560518883-ce09059eeffa?w=800&h=600&fit=crop',
  'Golf': 'https://images.unsplash.com/photo-1587174486073-ae5e5cff23aa?w=800&h=600&fit=crop',
  'Opieka zdrowotna': 'https://images.unsplash.com/photo-1519494026892-80bbd2d6fd0d?w=800&h=600&fit=crop',
  'Porównanie': 'https://images.unsplash.com/photo-1552664730-d307ca884978?w=800&h=600&fit=crop',
};
...
const CATEGORIES: Record<string, CategoryConfig> = {
  top10: {
    title: 'Listy Top 10',
    description: 'Wyselekcjonowane rankingi najlepszych ofert Costa Blanki',
    color: 'from-purple-500 to-indigo-600',
    bgColor: 'bg-purple-50',
    textColor: 'text-purple-600',
  },
  lifestyle: {
    title: 'Styl życia & Mieszkanie',
    description: 'Wszystko o życiu nad Morzem Śródziemnym - plaże, koszty utrzymania, opieka zdrowotna i codzienne życie',
    color: 'from-pink-500 to-orange-500',
    bgColor: 'bg-pink-50',
    textColor: 'text-pink-600',
  },
  buying: {
    title: 'Poradniki kupna',
    description: 'Przewodniki krok po kroku dotyczące kupowania nieruchomości w Hiszpanii',
    color: 'from-primary-600 to-primary-800',
    bgColor: 'bg-primary-50',
    textColor: 'text-primary-600',
  },
  areas: {
    title: 'Porównania regionów',
    description: 'Północ vs południe, plaża vs wnętrze - znajdź swoje idealne miejsce',
  ...
  golf: {
    title: 'Golf & Sport',
    description: 'Pola golfowe, obiekty i przewodniki aktywnego stylu życia',
...
function getArticles(): Article[] {
  // Primary directory for Polish articles
  const plArticlesDir: string = path.join(process.cwd(), 'src', 'content', 'articles', 'pl');

  const articles: Article[] = [];

  // Load only Polish articles — no English fallback
  if (fs.existsSync(plArticlesDir)) {
    const files: string[] = fs.readdirSync(plArticlesDir).filter((f: string) => f.endsWith('.json'));
    for (const file of files) {
      try {
        const content = JSON.parse(
          fs.readFileSync(path.join(plArticlesDir, file), 'utf-8')
        ) as Record<string, unknown>;
        const slug: string = file.replace('.json', '');
        articles.push({
          slug,
          title: (content.title as string) || 'Bez tytułu',
          excerpt: (content.excerpt as string) || '',
          category: (content.category as string) || 'Ogólne',
...
function translateCategory(englishCat: string): string {
  const map: Record<string, string> = {
    ...
    'Area Guide': 'Przewodnik regionu',
    'Buying Guide': 'Poradnik kupna',
    'Golf': 'Golf',
...
function groupArticles(articles: Article[]): GroupedArticles {
  ...
  const buying: Article[] = articles.filter(
    (a: Article) =>
      ...
      a.slug.includes('kupowanie')
  );

  const areas: Article[] = articles.filter(
    (a: Article) =>
      a.slug.includes('-vs-') ||
      a.category === 'Przewodnik regionu' ||
      a.category === 'Porównanie'
  );
...
function HeroArticle({ article }: HeroArticleProps): JSX.Element {
  const href: string = `/pl/blog/${article.slug}`;
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 right-0 p-8 md:p-12">
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="text-white/60 text-sm">{article.readTime} min czytania</span>
        </div>
      </div>
    </Link>
  );
    ...
    <section className="py-12">
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex items-center justify-between mb-8">
          ...
          {articles.length > 3 && (
            <Link
              href={`/pl/blog?category=${category}`}
...
export default function BlogPagePl(): JSX.Element {
  ...
  const breadcrumbs = breadcrumbSchema([
    ...
    { name: 'Blog', url: 'https://newbuildhomescostablanca.com/pl/blog/' },
  ]);

  const blogCollectionSchema = collectionPageSchema({
    name: 'Costa Blanca Poradniki: Kupować & Styl Życia',
    description: 'Poradniki ekspertów dotyczące kupowania nieruchomości i życia na Costa Blance, Hiszpania.',
    url: 'https://newbuildhomescostablanca.com/pl/blog/',
    items: articles.slice(0, 10).map((a: Article) => ({
      name: a.title,
      url: `https://newbuildhomescostablanca.com/pl/blog/${a.slug}/`,
    })),
  });
              ...
              <div className="lg:col-span-2">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <a
                    ...
                    className="bg-white/10 hover:bg-white/20 border border-white/20 rounded-sm p-4 text-center transition-colors"
                  >
                    <span className="text-white text-sm font-medium">Styl życia</span>
                  </a>
                  <a
                    href="#buying"
                    className="bg-white/10 hover:bg-white/20 border border-white/20 rounded-sm p-4 text-center transition-colors"
          ...
          <div className="max-w-7xl mx-auto px-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
              <div>
                ...
                <div className="text-warm-500 text-sm">Poradniki ekspertów</div>
              </div>
              <div>
                <div className="text-3xl font-light text-primary-900">300+</div>
                <div className="text-warm-500 text-sm">Słonecznych dni</div>
              </div>
              <div>
                <div className="text-3xl font-light text-primary-900">200km</div>
                <div className="text-warm-500 text-sm">Wybrzeże</div>
              </div>
              <div>
                <div className="text-3xl font-light text-primary-900">20+</div>
                <div className="text-warm-500 text-sm">Pola golfowe</div>
              </div>
            </div>
          </div>
        </section>
          ...
          <div className="max-w-7xl mx-auto px-6">
            <div className="text-center mb-12">
              ...
              <p className="text-warm-600">
                Przeglądaj naszą kompletną kolekcję poradników i artykułów
              </p>
            </div>
        ...
        <section className="py-16 bg-gradient-to-r from-accent-500 to-accent-600">
          <div className="max-w-4xl mx-auto px-6 text-center">
            ...
            <p className="text-white/90 mb-8 max-w-xl mx-auto">
              Otrzymuj nowe artykuły, aktualizacje rynku nieruchomości i porady
              od wewnątrz co tydzień.
            </p>
            <form
            ...
            <div className="grid md:grid-cols-2 gap-8">
              ...
              >
                ...
                <p className="text-warm-300 mb-4">
                  Odkryj 1000+ nowych domów na Costa Blance.
                </p>
              ...
              >
                ...
                <p className="text-white/90 mb-4">
                  Nasz lokalny zespół może odpowiedzieć na Twoje pytania.
                </p>
                <span className="text-white font-semibold">Skontaktuj się z nami →</span>
              </Link>
            </div>
          </div>
        </section>
