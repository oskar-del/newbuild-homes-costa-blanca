...
async function AutoGeneratedDevelopmentPage({ development }: { development: Development }) {
  const [units, builderDevelopments] = await Promise.all([
    getDevelopmentUnits(development.slug),
    getDevelopmentsByBuilder(development.developerSlug),
  ]);
      ...
      <main className="min-h-screen bg-warm-50">
        ...
        <section className="relative bg-primary-900 py-20">
          <div className="absolute inset-0">
            <Image src={development.mainImage} alt={development.name} fill className="object-cover opacity-30" priority unoptimized />
          </div>
          <div className="relative max-w-7xl mx-auto px-6">
            ...
            <p className="text-warm-300 text-lg mb-6">
              by <Link href={`/builders/${development.developerSlug}`} className="text-accent-400 hover:underline">{development.developer}</Link>
              {' • '}{development.town}, {development.region}
            </p>

            <div className="flex flex-wrap gap-4">
              <div className="bg-white/10 backdrop-blur px-5 py-3 rounded">
                <div className="text-2xl font-bold text-white">{formatPrice(development.priceFrom)}</div>
                <div className="text-warm-400 text-sm">From</div>
              </div>
              <div className="bg-white/10 backdrop-blur px-5 py-3 rounded">
                <div className="text-2xl font-bold text-white">{development.totalUnits}</div>
                <div className="text-warm-400 text-sm">Units</div>
              </div>
              <div className="bg-white/10 backdrop-blur px-5 py-3 rounded">
                <div className="text-2xl font-bold text-white">{development.bedroomRange}</div>
              ...
              {units.length > 0 ? (
                <div className="grid md:grid-cols-2 gap-5">
                  {units.map((unit: { id: string; ref: string; images: string[]; bedrooms: number | null; bathrooms: number | null; propertyType: string; price: number | null; size: number | null }) => (
                    <Link key={unit.id} href={`/properties/${unit.ref}`} className="group bg-white rounded-lg overflow-hidden border border-warm-200 hover:shadow-lg transition-all">
                      <div className="relative h-48">
                        <Image src={unit.images[0] || '/placeholder.jpg'} alt={`${unit.bedrooms} bed ${unit.propertyType}`} fill className="object-cover" unoptimized />
                      </div>
                      <div className="p-4">
                        <h3 className="font-semibold text-primary-900 group-hover:text-accent-600">{unit.bedrooms} Bed {unit.propertyType}</h3>
                        <p className="text-warm-500 text-sm">{unit.bathrooms} bath • {unit.size}m²</p>
